...
public class DualDARManager {
            ...
            try {
                ...
                if (fdFromPathForWrite != null && fdFromPathForWrite.fd != null) {
                    ...
                    if (processCommand == null
                            || !processCommand.getBoolean("dual_dar_response", true)) {
                ...
                if (fdFromPathForWrite != null
                        && (parcelFileDescriptor2 = fdFromPathForWrite.fd) != null) {
                    try {
    ...
    private FileInfo getFdFromPathForWrite(String str) {
        if (str != null && !str.isEmpty()) {
            ...
            try {
                return new FileInfo(
                        str.substring(str.lastIndexOf(47) + 1),
                        ParcelFileDescriptor.open(file, 536870912),
                        0L,
                        file.length());
            } catch (FileNotFoundException e) {
                e.printStackTrace();
                    ...
                    for (int i = 0; i < 5; i++) {
                        ...
                        if (!arrayList.isEmpty()) {
                            bundle.putParcelableArray(
                                    "CRYPTO_FILE_INFO",
                                    (Parcelable[]) arrayList.toArray(new FileInfo[0]));
                        }
                        Bundle processCommand =
                                processCommand(INSTALL_CLIENT_LIBRARY_REQUEST, bundle);
                        z2 =
                                processCommand != null
                                        && processCommand.getBoolean("dual_dar_response", true);
                        if (z2) {
    ...
    private Bundle processCommand(String str, Bundle bundle) {
        return KnoxProxyManager.getInstance(this.mContext)
                .relayMessage(DUALDAR_AGENT, DUALDAR_MGR_SERVICE, str, bundle);
    }

    private Bundle processCommandSecurely(String str, Bundle bundle) {
        return KnoxProxyManager.getInstance(this.mContext)
                .relayMessageSecurely(
                        DUALDAR_AGENT, DUALDAR_MGR_SERVICE, str, bundle, this.mSecureClientOutAPI);
    }
    ...
    public void establishSecureSession() {
        try {
            this.mSecureClientOutAPI =
                    KnoxProxyManager.getInstance(this.mContext)
                            .initializeSecureSession(
                                    DUAL_DAR_CLIENT, DUALDAR_AGENT, DUALDAR_MGR_SERVICE);
        } catch (Exception e) {
    ...
    public synchronized void setSecret(int i, List<Secret> list) {
        ...
        try {
            ...
            for (Secret secret : list) {
                try {
                    bArr =
                            this.mSecureClientOutAPI.encryptMessageFor(
                                    DUALDAR_MGR_SERVICE, secret.data);
                } catch (Exception e) {
            ...
            Log.d(
                    TAG,
                    "PUSH_SECRET_REQUEST response: "
                            + (processCommandSecurely != null
                                    ? processCommandSecurely.getBoolean("dual_dar_response", true)
                                    : false));
        } catch (Exception e2) {
            e2.printStackTrace();
        }
        ...
        try {
            KnoxProxyManager.getInstance(this.mContext)
                    .terminateSecureSession(
                            this.mSecureClientOutAPI, DUALDAR_AGENT, DUALDAR_MGR_SERVICE);
