...
public class PermissionChecker {
    ...
    public static void enforceKnoxAccessPermission(
            Context context, int i, int i2, String str, String str2)
                    ...
                    if (UserHandle.getUserId(i2) != 0) {
                        if (!isOrganizationOwnedDeviceWithManagedProfile(context)) {
                            ...
                            throw new SecurityException(
                                    "This API is works only with managedProfile(WPC)");
                        }
                    } else if (!hasDeviceOwner(context)) {
                        Log.e(TAG, "This API is works only with managed device(DO)");
                        throw new SecurityException(
                                "This API is works only with managed device(DO)");
                    }
                }
