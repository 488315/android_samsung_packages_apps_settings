...
public class SlowKeysPreferenceController extends TogglePreferenceController
        implements AccessibilityObservableController,
                AccessibilityUsingFunction,
                A11yStatusLoggingProvider {
    private Preference mPreference;

    public SlowKeysPreferenceController(Context context, String str) {
        super(context, str);
    }
                ...
                AccessibilityDialogUtils.createExclusiveDialog(
                        ...
                        new DialogInterface
                                .OnClickListener() { // from class:
                                                     // com.samsung.android.settings.accessibility.dexterity.controllers.SlowKeysPreferenceController$$ExternalSyntheticLambda0
                            @Override // android.content.DialogInterface.OnClickListener
                            public final void onClick(DialogInterface dialogInterface, int i) {
                                SlowKeysPreferenceController.this.lambda$showExclusiveDialog$0(
                                        dialogInterface, i);
                            }
                        },
                        null);
    ...
    public CharSequence getSummary() {
        ...
        SemLog.d("SlowKeysPreferenceController", "getSummary()");
        if (!getThreadEnabled()) {
            return this.mContext.getText(R.string.slow_keys_summary);
        }
        SemLog.d("SlowKeysPreferenceController", "getSummary( isSlowKeysEnabled)");
        return Float.compare(f, 1.0f) == 0
                ? this.mContext.getString(
                        R.string.hold_second,
                        NumberFormat.getInstance(Locale.getDefault()).format(f))
                : f % 1.0f == 0.0f
                        ? this.mContext.getString(
                                R.string.hold_seconds,
                                NumberFormat.getInstance(Locale.getDefault()).format(f))
                        : this.mContext.getString(
                                R.string.hold_seconds,
                                String.format(Locale.getDefault(), "%.1f", Float.valueOf(f)));
    }

    @Override // com.samsung.android.settings.accessibility.base.controller.AccessibilityObservableController
    public List<Uri> getUriList() {
        return List.of(Settings.Secure.getUriFor("accessibility_slow_keys"));
    }
    ...
    public CharSequence getUsingFunctionTitle(Context context) {
        return context.getText(R.string.slow_keys);
    }
    ...
    public boolean setChecked(boolean z) {
        SemLog.d("SlowKeysPreferenceController", "setChecked()");
    ...
    public boolean setCheckedForAction(boolean z) {
        if (!z || !AccessibilityExclusiveUtil.isExclusiveTaskEnabled(this.mContext, "slow_keys")) {
            return super.setCheckedForAction(z);
        }
        AccessibilityDialogUtils.startDialogActivity(this.mContext, 2, "slow_keys");
        return false;
    }
